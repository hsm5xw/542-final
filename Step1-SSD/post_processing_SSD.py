...
def get_targets(clip_path):
    ...
    '''
    clip_info, dimension = get_clip_info(clip_path)  # called from SSD500.py
    clip_classes = []
    ...
    if (len(class_count) > 0):
        ...
        max_classes = OrderedDict(Counter(class_count).most_common(2))  # return 2 most common classes
...
if __name__ == "__main__":
    ...
    for clip_path in test_folders:      # loop through each test case (e.g. blackswan)
        have_class ,max_classes, class_count, clip_info, [height, width]= get_targets(clip_path)

        if (have_class):
            for target_class in max_classes:        # max_classes: 2 most common classes
                print(clip_path+":",class_dict[target_class])

            for frame in clip_info:     # loop through each frame
                frame_info = frame["frame_info"]
                ...
                for target_class in max_classes:    # max_classes: 2 most common classes
                    ...
                    if (Bbox_info["have_box"]):
                        willoutput = True
                        max_box = Bbox_info["biggest_box"] # [ymin, xmin, ymax, xmax] from 0 to 1?
                ...
                if (willoutput):
                    ...
                    cv2.imwrite(outfile_path, out_img)  # save output image
        else:
