...
def train_label_reader(inputrgb_queue, inputmsk_queue, label_queue):  # called from next_batch() function
...
vgg_fcn = fcn8_vgg_ours.FCN8VGG()       # FCN
with tf.name_scope("content_vgg"):
    vgg_fcn.build(batch_images, train = True, debug=True)


labels = tf.placeholder("int32", [None, HEIGHT, WIDTH])
loss = fcn8_vgg_ours.pixel_wise_cross_entropy(vgg_fcn.upscore32, labels, num_classes = 2)   # FCN loss to minimize
...
train_set, val_set = load_training_set(train_file, valid_file)    # see function defined above

inputrgb_queue = tf.train.string_input_producer(train_set[RGB_IMG], shuffle=False)  # output strings (e.g. filenames) to a queue for an input pipeline.
...
batch = next_batch(inputrgb_queue, inputmsk_queue, label_queue)   # see function defined above
...
try:
    for i in range(start, NUM_EPOCHS):  # repeat training until reaching NUM_EPOCHS
        print('epoch %d ...'%i)

        tmp = batch.eval()
        input_ = tmp[:,:,:,:4]    # fetch batch_images and labels
        label_ = tmp[:,:,:,4]     
        start_time = time.time()
        train_step.run(feed_dict={batch_images:input_, labels:label_})  # run training

        print('Train: time elapsed: %.3fs.'%(time.time()-start_time))

        if i % 40 == 0 and i > start:
            summary = sess.run(merged_summary, feed_dict={batch_images:input_, labels:label_}) # run summary for every 40 iterations
