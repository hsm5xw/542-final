...
img1 = scp.misc.imread("DAVIS/images/"+name+".jpg")                 # read image 
...
with tf.Session() as sess:                          # open session
    ...
    with tf.name_scope("content_vgg"):
        vgg_fcn.build(batch_images, debug=True)     # build the vgg_fcn ("VGG OBJECT") for finetuning (FCN8 VGG here)
    ...
    if ckpt and ckpt.model_checkpoint_path:
        start = int(ckpt.model_checkpoint_path.split("-")[1])   # read start from checkpoint state
